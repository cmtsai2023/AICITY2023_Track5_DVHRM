echo "start to extract frames from test videos..."
starttime=`date +%s`
#get video list
videos_list=()
videos_list+=("$@")
...
for videoname in "${videos_list[@]}"; do
    read -u6
    {
        savepath="./test/${videoname}"
        mkdir -p ${savepath}
        ffmpeg -i ${data_root}/${videoname}.mp4 -q:v 2 ${savepath}/%d.jpg -hide_banner
